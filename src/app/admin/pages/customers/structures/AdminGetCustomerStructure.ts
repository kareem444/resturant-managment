...
export const AdminGetCustomerStructure =
    (): IAdminDetailsStatusContainerProps => {
        ...
        const tableContent: ITableContent = {
            ...
            items: data || [],
            selectors: {
                1: (item: IAdminCustomerModel) => item.mobile,
                2: (item: IAdminCustomerModel) => item.taxNumber,
                3: (item: IAdminCustomerModel) => item.address,
                4: (item: IAdminCustomerModel) => moment(item.createdAt).format('D MMM'),
            },
