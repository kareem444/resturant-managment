...
export class AdminDeliveryRepo {
    static createDelivery = (delivery: IAdminDeliveryInputs): Promise<IAdminDeliveryModel> => {
        return AsyncHelper.createPromise(async () => {
            ...
            if (!!delivery.image) {
                image = await FireStorageHelper.uploadFile(delivery.image, { directory: 'delivery' })
            }
    ...
    static getDelivery = async (): Promise<IAdminDeliveryModel[] | undefined> => {
        return AsyncHelper.createPromise(async () => {
    ...
    static updateDelivery = (deliveryId: string, delivery: IAdminDeliveryInputs): Promise<IAdminDeliveryModel> => {
        return AsyncHelper.createPromise(async () => {
            const branches: IAdminBranchModel[] = await AdminBranchesRepo.getBranches();
            const branch = branches?.find(branch => branch.id === delivery.branchId);
